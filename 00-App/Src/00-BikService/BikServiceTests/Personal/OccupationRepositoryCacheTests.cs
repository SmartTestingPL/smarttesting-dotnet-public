...
public class OccupationRepositoryCacheTests
{
  ...
  public void ShouldRetrieveOccupationScoresFromCache()
  {
    ...
    DropOccupationToScoreTable();

    GetOccupationScores().Should().ContainKeys(
      expectedValues.Cast<PersonalInformation.Occupations?>(),
  ...
  private Dictionary<PersonalInformation.Occupations?, Score> GetOccupationScores()
  {
    ...
    var repository = scope.ServiceProvider.GetRequiredService<CachedOccupationRepository>();
    return repository.GetOccupationScores();
  }

  private void DropOccupationToScoreTable()
  {

    using var scope = _container.CreateAsyncScope();
    var repository = scope.ServiceProvider.GetRequiredService<EfCoreOccupationRepository>();
    var tableName = repository.Model.FindEntityType(typeof(OccupationToScore)).GetSchemaQualifiedTableName();
    repository.Database.ExecuteSqlRaw($"drop table \"{tableName}\"");
  }
