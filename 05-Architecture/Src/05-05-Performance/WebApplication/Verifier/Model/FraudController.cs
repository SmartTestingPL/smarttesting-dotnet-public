...
public class FraudController : ControllerBase
{
  ...
  [HttpPost("fraudCheck")]
  public async Task<IActionResult> FraudCheck(Customer person, CancellationToken cancellationToken)
  {
    _logger.LogInformation($"Received a verification request for person [{person}]");
    var result = await _customerVerifier.Verify(person, cancellationToken);
